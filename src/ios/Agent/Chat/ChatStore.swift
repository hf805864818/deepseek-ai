...
actor ChatStore {
    ...
    func interruptedSessionsWithTailDate() -> [String: Date] {
        ...
        guard sqlite3_prepare_v2(db, sql, -1, &stmt, nil) == SQLITE_OK else {
            ...
            logger.error("[ChatStore.interruptedSessionsWithTailDate] prepare failed: \(err)")
            return [:]
        }
        ...
        logger.info("[ChatStore.interruptedSessionsWithTailDate] scanned \(lastBySession.count) sessions → \(result.count) interrupted")
        return result
    }

    /// Backward-compatible wrapper: returns just the id set.
    /// New callers should prefer `interruptedSessionsWithTailDate()` so the
    /// badge store can stamp with the real interruption time.
...
extension ChatStore {
    ...
    /// backup to get it back" case, which is a primary reason to have backups
    /// at all. Restoring is an explicit user action, so it outranks the guard.
