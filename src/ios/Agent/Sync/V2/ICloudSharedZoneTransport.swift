...
final class ICloudSharedZoneTransport: NSObject, SyncTransport {
    ...
    static let containerIdentifier = "iCloud.com.lobster.app"
    ...
    override init() {
        // [T-ios-icloud-lazy-init] Do NOT initialize CKContainer in init().
        // CKContainer(identifier:) can throw NSException in certain environments
        // (jailbreak, missing entitlements, network issues) before Swift's
        // do/catch can intercept it, resulting in objc_terminate → SIGTRAP.
        //
        // Instead, we defer container creation until start() is called, where
        // we can properly catch and handle any errors. This allows the app to
        // launch successfully even if iCloud is temporarily unavailable.
        self.container = nil
        self.stateURL = FileManager.default
            .urls(for: .libraryDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("MinisChat/cloud-sync-v2/state-v2.bin")
        super.init()
    }

    // MARK: - Lazy Container Initialization

    /// Returns the CKContainer, initializing it on first use.
    /// This method is thread-safe and can be called from any actor.
    func getContainer() -> CKContainer? {
        ...
        do {
            container = try CKContainer(identifier: Self.containerIdentifier)
            self.container = container
            logger.info("[SyncTransport] CKContainer initialized successfully for \(Self.containerIdentifier)")
        } catch let error as CKError {
            // CloudKit-specific error (e.g., container not found, permission denied)
            logger.error("[SyncTransport] CKContainer initialization failed with CloudKit error: \(error.localizedDescription)")
            // Fall back to default container as a last resort
            container = CKContainer.default()
            self.container = container
            logger.warning("[SyncTransport] Falls back to default CKContainer")
        } catch let error {
            // Other errors (e.g., invalid identifier, network issues)
            logger.error("[SyncTransport] CKContainer initialization failed: \(error.localizedDescription)")
    ...
    func isICloudAvailable() -> Bool {
        ...
        return container.accountStatus { accountStatus, accountError in
            ...
            if let error = error {
                logger.warning("[SyncTransport] iCloud account status check failed: \(error.localizedDescription)")
            }
    ...
    func start() async throws {
        ...
        // [T-ios-icloud-lazy-init] Initialize CKContainer here instead of in init().
        // This avoids the crash in jailbroken environments where CKContainer
        // initialization can throw NSException before Swift's do/catch can catch it.
        if self.container == nil {
            logger.info("[SyncTransport] Initializing CKContainer lazily...")
            do {
                self.container = try CKContainer(identifier: Self.containerIdentifier)
                logger.info("[SyncTransport] CKContainer initialized successfully")
            } catch {
                logger.error("[SyncTransport] CKContainer initialization failed: \(error.localizedDescription)")
                // Re-throw as a generic error that can be caught by SyncV2Bootstrap.runGuarded
                throw error
            }
        ...
        guard let container = getContainer() else {
            logger.error("[SyncTransport] cannot start: no container")
            return
        }
